...
async function publishTextToX(text, options) {
	...
	if (postText === "") throw new Error("X text must not be empty.");
	const url = "https://api.x.com/2/tweets";
	try {
		const response = await (options.fetchFn ?? fetch)(url, {
			body: JSON.stringify({ text: postText }),
			headers: {
				Authorization: createOAuthAuthorizationHeader("POST", url, options.credentials),
				"Content-Type": "application/json"
			},
			method: "POST"
		});
		if (!response.ok) throw new DeliveryResponseError(`X delivery failed with HTTP ${response.status}.`);
	} catch (error) {
		if (error instanceof DeliveryResponseError) throw error;
		throw new Error(`X delivery has an unknown outcome and will not be retried automatically: ${toErrorMessage(error)}`);
	}
}
function createOAuthAuthorizationHeader(method, url, credentials) {
	...
	const parameters = [...target.searchParams.entries(), ...Object.entries(oauthParameters)].map(([key, value]) => [oauthEncode(key), oauthEncode(value)]).sort(([leftKey, leftValue], [rightKey, rightValue]) => {
...
function oauthEncode(value) {
	return encodeURIComponent(value).replace(/[!'()*]/g, (character) => `%${character.codePointAt(0)?.toString(16).toUpperCase()}`);
}
function getInput(name) {
	const value = process.env[`INPUT_${name.toUpperCase().replaceAll(" ", "_")}`];
	if (value === void 0 || value.trim() === "") throw new Error(`Missing required input: ${name}`);
	return value.trim();
}
...
async function main() {
	await publishTextToX(getInput("text"), { credentials: {
...
//#endregion
export { getInput, publishTextToX };
